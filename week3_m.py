...
train_dataset = MNISTCSVLoader("Data/sample_Data/mnist_train_small.csv")
test_dataset = MNISTCSVLoader("Data/sample_Data/mnist_test.csv")
...
class NeuralNet(nn.Module): #This creates a custom neural network by inheriting from nn.Module, which is the base class for all PyTorch models.
    def __init__(self): #__init__() is a constructor function that runs when we create a model object
        super(NeuralNet, self).__init__() #super() is used to call the constructor of the parent class (nn.Module)
        # TODO: Create the first fully connected layer (input: 28*28 pixels, output: 128 neurons)
        # Takes a flattened image of 28×28 pixels as input (since MNIST images are 28×28).
        # Outputs 128 neurons (a compressed representation of the image)
        self.fc1 = nn.Linear(28 * 28, 128) #nn.Linear() creates a fully connected layer with the specified input and output size

        # TODO: Create the second fully connected layer (input: 128 neurons, output: 64 neurons)
        # further compressed information
        self.fc2 = nn.Linear(128, 64)

        # TODO: Create the third (output) fully connected layer (input: 64 neurons, output: 10 classes)
        # Outputs 10 neurons (one for each digit 0-9)
        # The final output must have 10 values because we are classifying digits (0-9).
        # Each neuron represents the confidence score for a particular digit.
        ...
        # This introduces non-linearity to the model, allowing it to learn complex patterns in the data.
        self.relu = nn.ReLU()
